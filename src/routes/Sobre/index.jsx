import { SobreMain } from "../../styled";

export default function Sobre(){

    return(
        <>
            <SobreMain>
            <div className="container">
        <h1>Sobre Nós</h1>
        <div className="highlight-box">
          <h2>História da Olympia Sport</h2>
          <p>
            A Olympia nasceu do sonho de Josiane Souza, uma apaixonada por esportes olímpicos
            desde a infância. Criada em uma pequena cidade do interior, Josiane cresceu admirando as
            Olimpíadas, mas sempre sentiu na pele a dificuldade de encontrar equipamentos adequados
            para as modalidades que amava. Praticante dedicada de ginástica artística e tiro com arco,
            ela muitas vezes precisava importar seus próprios equipamentos por falta de opções no
            Brasil. Essa frustração se transformou em motivação, e Josiane decidiu que um dia mudaria
            essa realidade.
          </p>
        </div>

        <div className="highlight-box">
          <h2>Inspiração e Criação</h2>
          <p>
            Determinada a preencher essa lacuna, ela embarcou em uma jornada empreendedora. Em suas
            viagens internacionais para competir e assistir a grandes eventos esportivos, Josiane
            começou a se inspirar nas lojas especializadas em esportes olímpicos que encontrava no
            exterior. Ao perceber o quanto esse tipo de loja era raro no Brasil, surgiu a ideia da
            Olympia Sport, uma loja voltada exclusivamente para esportes olímpicos, com o objetivo de
            fornecer produtos de alta qualidade para atletas e entusiastas de todas as modalidades.
          </p>
        </div>

        <div className="highlight-box">
          <h2>Nosso Propósito</h2>
          <p>
            Com a Olympia Sport, Josiane deseja não só facilitar o acesso aos artigos esportivos, mas
            também contribuir para o crescimento do esporte olímpico no país. A loja oferece uma ampla
            gama de produtos, atendendo desde esportes populares, como natação e atletismo, até os
            menos conhecidos, como luta greco-romana e badminton. Para Josiane, cada venda é uma chance
            de apoiar um atleta em potencial e alimentar o sonho de ver mais brasileiros brilhando nos
            Jogos Olímpicos.
          </p>
        </div>

        <h2>Missão</h2>
        <p>
          Oferecer produtos de alta qualidade para todas as modalidades esportivas olímpicas,
          incentivando a prática desses esportes e contribuindo para o desenvolvimento dos atletas
          brasileiros. Nossa missão é fazer com que os esportes olímpicos sejam mais acessíveis e
          populares no Brasil.
        </p>

        <h2>Visão</h2>
        <p>
          Ser a loja referência em artigos esportivos olímpicos no Brasil, criando uma comunidade de
          atletas e entusiastas que compartilham a paixão pelo esporte e pela superação de limites. A
          Olympia visa apoiar o crescimento das modalidades olímpicas e impulsionar o esporte
          no país.
        </p>

        <h2>Valores</h2>
        <ul>
          <li>
            <strong>Paixão pelo Esporte:</strong> Amamos o que fazemos e acreditamos no poder
            transformador do esporte na vida das pessoas.
          </li>
          <li>
            <strong>Compromisso com a Qualidade:</strong> Oferecemos apenas produtos que garantem
            desempenho e segurança aos nossos clientes.
          </li>
          <li>
            <strong>Inclusão e Diversidade:</strong> Valorizamos todas as modalidades esportivas e
            acreditamos que o esporte deve ser acessível para todos.
          </li>
          <li>
            <strong>Incentivo ao Desenvolvimento:</strong> Apoiamos o crescimento dos atletas e o
            desenvolvimento dos esportes olímpicos no Brasil.
          </li>
          <li>
            <strong>Sustentabilidade:</strong> Priorizamos práticas que respeitam o meio ambiente,
            alinhadas com os valores olímpicos de respeito e preservação.
          </li>
        </ul>
      </div>
            </SobreMain>
        </>
    )
}